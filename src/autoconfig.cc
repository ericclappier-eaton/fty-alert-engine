...
#define ConfigurableDevices_GUARD std::lock_guard<std::recursive_mutex> guard(_configurableDevicesMutex)
...
void Autoconfig::onSend(fty_proto_t** message)
{
    ...
    if ((strcmp(fty_proto_operation (*message), FTY_PROTO_ASSET_OP_UPDATE) == 0) &&
        !currentInfo.empty() && (currentInfo == *message))
    {
    ...
    if (!currentInfo.empty() && (0 != strcmp(fty_proto_ext_string(*message, "update_ts", ""), currentInfo.update_ts.c_str()))) {
